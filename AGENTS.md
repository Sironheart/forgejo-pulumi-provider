...
- `schema.json` and SDKs under `sdk/nodejs`, `sdk/go`, `sdk/dotnet`, and `sdk/java` are generated by Forgejo Actions and are not committed. `sdk/dotnet/logo.png` is the only tracked SDK-tree asset.
...
- Export schema for local inspection: `mise run schema` writes ignored `schema.json` from the built plugin.
- Generate SDKs for local inspection: `mise run sdk`; output is ignored. Limit languages with `FORGEJO_SDK_LANGUAGES="nodejs go dotnet java"` or a subset.
...
- If a change affects provider types, config, descriptions, package metadata, or resources, Forgejo Actions generates fresh schema and SDK artifacts from that change.
- Build/codegen version defaults to `svu next`. Forgejo Actions passes its calculated version explicitly when generating release SDKs.
- Use `FORGEJO_PROVIDER_VERSION=0.0.0-dev` only for throwaway local experiments that you will not commit.
- Current executable package metadata names the Node SDK `@sironheart/pulumi-forgejo-provider`; trust `internal/provider/provider.go` and generated CI artifacts over README examples if they disagree.
...
- PR artifact generation exports and uploads `schema.json` plus generated SDKs. Release generation uploads SDKs, GoReleaser exports the release schema, and package jobs download the same-run SDK artifact before publishing.
